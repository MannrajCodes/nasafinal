...
import { OrbitControls, Stars, Html, Environment, useTexture, Line } from "@react-three/drei"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { Zap, Target, Wrench, Eye } from "lucide-react"
import * as THREE from "three"
import { nasaDebrisClient, type DebrisObject, type DebrisStatistics } from "@/lib/nasa-debris"
import { type TLEData, type ISSPosition } from "@/lib/nasa-official"
...
function Earth({ timeScale }: { timeScale: number }) {
  ...
  const setAuroraRef = (index: number) => (mesh: THREE.Mesh | null) => {
    if (mesh) {
      auroraRefs.current[index] = mesh
    }
  }
  ...
  useFrame((state) => {
    ...
    auroraRefs.current.forEach((aurora) => {
      if (aurora) {
        aurora.rotation.y += 0.005 * timeScale
  ...
  return (
    <group>
      ...
      <mesh ref={setAuroraRef(0)} scale={1.03} position={[0, 1.8, 0]}>
      ...
      <mesh ref={setAuroraRef(1)} scale={1.03} position={[0, -1.8, 0]} rotation={[Math.PI, 0, 0]}>
        <ringGeometry args={[0.3, 0.8, 32]} />
        <meshBasicMaterial
          color="#FF0088"
          transparent
...
function Satellite({
  ...
  useFrame((state) => {
    if (satelliteRef.current) {
      ...
      satelliteRef.current.rotation.y += 0.02 * timeScale
...
function OrbitalPath({
  ...
  const points = useMemo(() => {
    ...
    for (let i = 0; i <= 64; i++) {
      ...
      generated.push(new THREE.Vector3(Math.cos(angle) * radius, 0, Math.sin(angle) * radius))
    }
    return generated
  }, [radius])
...
function SpaceBackground() {
  ...
  useFrame((state) => {
    ...
    if (nebulaRef1.current) {
      ...
      const material = nebulaRef1.current.material
      if (!Array.isArray(material)) {
    ...
    if (nebulaRef2.current) {
      ...
      const material = nebulaRef2.current.material
      if (!Array.isArray(material)) {
        material.opacity = 0.06 + Math.cos(time * 0.4) * 0.02
      }
    }

    if (nebulaRef3.current) {
      nebulaRef3.current.rotation.z += 0.0005
      const material = nebulaRef3.current.material
      if (!Array.isArray(material)) {
...
function OrbitalScene({
  ...
  const [debrisStats, setDebrisStats] = useState<DebrisStatistics | null>(null)
  const [realSatellites, setRealSatellites] = useState<TLEData[]>([])
  const [issPosition, setISSPosition] = useState<ISSPosition | null>(null)

  const loadNASAOfficialData = useCallback(async () => {
    try {
      // Load multiple NASA datasets
      const [debrisData, stats, tleResponse, issResponse] = await Promise.all([
        nasaDebrisClient.getDebrisData({ limit: 50, altitude: [400, 2000] }),
        nasaDebrisClient.getDebrisStatistics(),
        fetch("/api/nasa/tle").then(async (r) => (await r.json()) as ApiResponse<TLEData[]>),
        fetch("/api/nasa/iss").then(async (r) => (await r.json()) as ApiResponse<ISSPosition>),
      ])

      setNasaDebris(debrisData.data)
      setDebrisStats(stats)
      setRealSatellites(tleResponse.success && Array.isArray(tleResponse.data) ? tleResponse.data : [])
      setISSPosition(issResponse.success ? issResponse.data : null)
    } catch (error) {
      console.error("Failed to load NASA official data:", error)
    }
  }, [])

  const loadISSPosition = useCallback(async () => {
    try {
      const response = await fetch("/api/nasa/iss")
      const data = (await response.json()) as ApiResponse<ISSPosition>
      if (data.success) {
  ...
  useEffect(() => {
    ...
    void loadNASAOfficialData()

    // Update ISS position every 30 seconds
    const issInterval = setInterval(loadISSPosition, 30000)
    return () => clearInterval(issInterval)
  }, [camera, loadNASAOfficialData, loadISSPosition])

  const issTo3DPosition = (iss: ISSPosition | null): [number, number, number] => {
    if (!iss) return [0, 0, 0]
  ...
  return (
    <>
      ...
      {debrisStats && (
        <Html position={[-5, 3, -6]} transform occlude={false} distanceFactor={8}>
          <div className="bg-slate-900/80 backdrop-blur-sm border border-cyan-500/20 rounded-lg px-3 py-2 text-[10px] text-cyan-200 font-mono shadow-lg">
            <div className="text-xs font-semibold text-cyan-300 mb-1">NASA Debris Snapshot</div>
            <div>Total Objects: {debrisStats.totalObjects.toLocaleString()}</div>
            <div>Trackable: {debrisStats.trackableObjects.toLocaleString()}</div>
            <div>Collision Risk: {debrisStats.collisionRisk.toUpperCase()}</div>
            <div className="text-[9px] text-cyan-400/70 mt-1">
              Updated: {new Date(debrisStats.lastUpdate).toLocaleTimeString()}
            </div>
          </div>
        </Html>
      )}
...
export default function OrbitalSceneWrapper(props: OrbitalSceneProps) {
  return (
    <ClientOnlyWrapper>

...
type TransactionTypeOption = Transaction["type"] | ""
...
export function BlockchainDashboard() {
  ...
  const [newTransaction, setNewTransaction] = useState<TransactionFormState>(() => createEmptyTransactionForm())
...
const handleCreateTransaction = async () => {
    ...
    if (newTransaction.powerAmount) data.powerAmount = newTransaction.powerAmount

    if (!validateTransactionData(transactionType, data)) {
      toast({
        title: "Incomplete payload",
        description: "Provide all required fields for the selected transaction type.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      setNewTransaction(createEmptyTransactionForm())
      await loadData()
    } catch (error) {
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setNewTransaction({ ...newTransaction, type: value as TransactionTypeOption })
                    }
                  >
                    <SelectTrigger>
  ...
  />
                ...
                {newTransaction.type === "power_transfer" && (
                  <>
                    <div>
                      <Label htmlFor="power-amount">Power Amount</Label>
                      <Input
                        id="power-amount"
                        value={newTransaction.powerAmount}
                        onChange={(e) => setNewTransaction({ ...newTransaction, powerAmount: e.target.value })}
                        placeholder="250 kWh"
                      />
                    </div>
                    <div>
